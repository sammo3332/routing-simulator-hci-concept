...
export default function FileUploader({ onFile, disabled = false }) {
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept=".json,.xml,.graphml" disabled={disabled} style={{ display:"none" }} onChange={e => handle(e.target.files[0])} />
      ...
      {fname
        ...
        : <>
            <div style={{ fontSize:11, fontWeight:700, color:"#7a8499", marginBottom:3 }}>TopoHub-JSON / SNDlib-XML / GraphML</div>
            <div style={{ fontSize:9, color:"#3e4860" }}>Drag & Drop oder klicken</div>
            <div style={{ marginTop:6, display:"inline-flex", gap:4 }}>
              {[".json",".xml",".graphml"].map(ext => (
